...
Type objective_function<Type>::operator() () 
{
  ...
  DATA_INTEGER(centered); // == 0 centered, == 1 non-centered
  ...
  PARAMETER_ARRAY(eps_at); // Random Effects Weight-at-age array
  ...
  // LIKELIHOOD SECTION ------------------------
  array<Type> mu_at(eps_at.rows(), eps_at.cols()); // Mean weight at age across time
  array<Type> Y_at(eps_at.rows(), eps_at.cols()); // Time-varying WAA
  ...
  for(int a = 0; a < X_at.rows(); a++) {
    for(int t = 0; t < X_at.cols(); t++) {
      ...
      if(centered == 0) Y_at(a,t) = mu_at(a,t) * exp(eps_at(a,t)); // centered deviations
      if(centered == 1) Y_at(a,t) = mu_at(a,t) * exp((sqrt(exp(log_sigma2)) * eps_at(a,t))); // non-centered deviations
    } // t loop
  } // a loop
  ...
  for(int a = 0; a < X_at.rows(); a++) {
    for(int t = 0; t < X_at.cols(); t++) {
      if( !isNA(X_at(a,t)) ){
        if(Xsd_at(a,t) > 0) jnLL -= dnorm(log(X_at(a,t)), log(Y_at(a,t)), Xsd_at(a,t), true); // only evaluate if sd > 0
  ...
  if(centered == 0) {
    jnLL += GMRF(Q_sparse)(eps_at); // centered
  }
  
  if(centered == 1) {
    Eigen::SparseMatrix<Type> Q_sparse_centered = pow(exp(log_sigma2), -1) * Q_sparse;
    jnLL += GMRF(Q_sparse_centered)(eps_at);
  } // non-center
